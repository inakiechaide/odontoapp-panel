...
import { Package, Plus, AlertTriangle, Search, Edit2, X, Check } from 'lucide-react'
...
export default function InventarioPage() {
  ...
  const [form, setForm] = useState({ nombre:'', categoria:'MATERIAL_CLINICO', unidad:'unidad', stockActual:0, stockMinimo:5, proveedor:'', precioUnitario:'' })
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({queryKey:['inventory']}); setEditing(null); toast.success('Item actualizado') }
  })
  ...
  const handleSubmit = () => {
    const d = {...form, stockActual: Number(form.stockActual), stockMinimo: Number(form.stockMinimo), precioUnitario: form.precioUnitario ? Number(form.precioUnitario) : undefined}
  ...
  const startEdit = (item: Item) => {
    setEditing(item)
    setForm({nombre: item.nombre, categoria: item.categoria, unidad: item.unidad, stockActual: item.stockActual, stockMinimo: item.stockMinimo, proveedor: item.proveedor??'', precioUnitario: item.precioUnitario?.toString()??''})
    setShowForm(true)
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=>{setEditing(null);setForm({nombre:'',categoria:'MATERIAL_CLINICO',unidad:'unidad',stockActual:0,stockMinimo:5,proveedor:'',precioUnitario:''});setShowForm(true)}}
      ...
      {alertas && alertas.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-amber-800">Stock bajo en {alertas.length} item{alertas.length>1?'s':''}</p>
            <p className="text-sm text-amber-600 mt-0.5">{alertas.map(a=>a.nombre).join(', ')}</p>
          </div>
        </div>
      )}
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={cn('font-bold text-base', bajo ? 'text-red-500' : 'text-gray-800')}>{item.stockActual}</span>
                    </td>
